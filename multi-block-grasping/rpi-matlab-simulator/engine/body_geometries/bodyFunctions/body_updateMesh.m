...
function B = body_updateMesh( B )
    ...
    % Update t vectors 
    for e =1:B.num_edges
        E = B.verts_local(B.edges(e,2),:) - B.verts_local(B.edges(e,1),:);  
        B.tvecs(e,1:3) = cross3(B.face_norms(B.edges(e,3),:), E);  % t1
        B.tvecs(e,4:6) = cross3(E, B.face_norms(B.edges(e,4),:));  % t2
        
        % This may not be strictly necessary, but let's normalize the t vecs
        B.tvecs(e,1:3) = B.tvecs(e,1:3) / norm(B.tvecs(e,1:3));
